...
router = APIRouter(prefix="/auth", tags=["2. App Auth & Security (v2)"])
...
async def verify_mpin(payload: MpinVerifyRequest):
    """
    Direct 4/6-digit MPIN verification for instant customer app unlock.
    """
    if payload.mpin not in ["1234", "0000", "5678"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def verify_biometric(payload: BiometricVerifyRequest):
    """
    Direct hardware biometric unlock for instant app access.
    """
    customer = mock_db.customers.get(payload.cif or "CIF100001", mock_db.customers["CIF100001"])
    session = AuthSessionData(
        cif=customer["cif"],
...
# --- NEW IN V2 ---
@router.post("/logout", response_model=BaseApiResponse, summary="Customer Session Logout (Server-Side Invalidation)")
async def customer_logout(authorization: Optional[str] = Header(None, description="Bearer Token")):
    """
    Invalidates the active customer authentication session server-side.
    Revokes the JWT access token and cleans up device push tokens.
    """
    return BaseApiResponse(
        success=True,
        response_code="BB-200",
        message="Session invalidated successfully. You have been logged out of Bharat Bank Mobile."
    )
